$(document).ready(function () {
  const commonSettings = {
    ...
    nav: true,
    navText: $("body").hasClass("ar") ? ['<i class="br-arrowNtail-left-ic"></i>', '<i class="br-arrowNtail-right-ic"></i>'] : ['<i class="br-arrowNtail-right-ic"></i>', '<i class="br-arrowNtail-left-ic"></i>'],
  };
...
function startCountersOnScroll() {
  ...
  const observer = new IntersectionObserver(
    ...
    { threshold: 0.5 }
  ); // تحديد أن العنصر يظهر بنسبة 50% على الأقل
...
playButton.addEventListener("click", () => {
  if (playButton.classList.contains("fi-pause")) {
    playButton.classList.remove("fi-pause");
    playButton.classList.add("fi-play");
    playButton.style.setProperty("--progress", "0deg"); // reset progress
  } else {
    playButton.classList.remove("fi-play");
    playButton.classList.add("fi-pause");
    playButton.style.setProperty("--progress", "91deg"); // set progress to 91
  }
});
